        ...
        <div class="py-8">
            <div class="flex items-center mb-6 space-x-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kurangi stok untuk produk yang digunakan atau dijual.</p>
                </div>
            </div>
            ...
            <form action="{{ route('manajergudang.stock.out.store') }}" method="POST" class="grid grid-cols-1 gap-8 mt-8 lg:grid-cols-3">
                ...
                <div class="p-6 space-y-6 bg-white rounded-lg shadow-md lg:col-span-2 dark:bg-slate-800">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            <label for="quantity" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Jumlah Keluar <span class="text-red-500">*</span></label>
                            <input type="number" id="quantity" name="quantity" x-model.number="quantity" @change="validateQuantity()" placeholder="1" min="1" :max="currentStock" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white" required>
                        </div>
                        <div>
                    ...
                    <div>
                        <label for="notes" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Tujuan / Catatan (Opsional)</label>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky p-6 bg-white rounded-lg shadow-md top-24 dark:bg-slate-800">
                        <div x-show="!selectedProductId" class="py-10 text-center text-gray-500 dark:text-gray-400">
                            <i class="mb-3 text-4xl fas fa-box-open"></i><p>Pilih produk untuk melihat informasi stok.</p>
                        </div>
                        <div x-show="selectedProductId" x-cloak class="space-y-4">
                            ...
                            <div class="pt-4 border-t dark:border-slate-700">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-2xl font-bold" :class="finalStock < 0 ? 'text-red-500' : 'text-blue-600 dark:text-blue-400'" x-text="`${finalStock} ${unit}`"></span>
                                </div>
                            </div>
                            <div class="pt-6">
                                <button type="submit" class="flex items-center justify-center w-full px-6 py-3 font-semibold text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed" :disabled="finalStock < 0 || quantity < 1">
                                    <i class="mr-2 fas fa-save"></i>Simpan Transaksi
                                </button>
                                <p x-show="finalStock < 0" class="mt-2 text-xs text-center text-red-500">Stok tidak boleh negatif!</p>
                            </div>
                        </div>
                    </div>
                </div>
