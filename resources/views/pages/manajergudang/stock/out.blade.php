...
@section('title', 'Ajukan Permintaan Barang Keluar')

@push('scripts')
{{-- Script Alpine.js untuk interaktivitas form --}}
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('stockOutForm', () => ({
            ...
            quantity: parseInt('{{ old('quantity', 1) }}'),
...
@section('content')
<div class="p-4 sm:p-6 lg:p-8" x-data="stockOutForm" x-init="init()">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 md:text-3xl dark:text-white">Ajukan Permintaan Barang Keluar</h1>
                <p class="mt-1 text-gray-600 dark:text-gray-400">Stok akan berkurang setelah permintaan ini diproses oleh Staff Gudang.</p>
            </div>
        </div>
    </div>
    
    <form action="{{ route('manajergudang.stock.out.store') }}" method="POST" class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        @csrf
        ...
        <div class="p-6 space-y-6 bg-white border border-gray-200 shadow-sm lg:col-span-2 rounded-xl dark:bg-slate-800 dark:border-slate-700">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <label for="quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Keluar <span class="text-red-500">*</span></label>
                    <input type="number" id="quantity" name="quantity" x-model.number="quantity" @change="validateQuantity()" min="1" :max="currentStock" class="w-full px-4 py-2 text-sm text-gray-900 bg-gray-50 border-gray-300 rounded-lg dark:text-white dark:bg-slate-700 dark:border-slate-600 focus:ring-cyan-500 focus:border-cyan-500" required>
                </div>
                <div>
            ...
            <div>
                <label for="notes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tujuan Penggunaan / Catatan</label>
                <textarea id="notes" name="notes" rows="3" placeholder="Contoh: Digunakan untuk proyek Renovasi Gedung A" class="w-full px-4 py-2 text-sm text-gray-900 bg-gray-50 border-gray-300 rounded-lg dark:text-white dark:bg-slate-700 dark:border-slate-600 focus:ring-cyan-500 focus:border-cyan-500">{{ old('notes') }}</textarea>
            </div>
        </div>
            ...
            <div class="sticky p-6 bg-white border border-gray-200 shadow-sm top-24 rounded-xl dark:bg-slate-800 dark:border-slate-700">
                ...
                <div x-show="currentProduct" x-cloak class="mt-4 space-y-4">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-slate-700">
                        ...
                        <span class="text-lg font-bold text-gray-900 dark:text-white" x-text="`${currentStock} ${currentProduct?.unit}`"></span>
                    </div>
                    <div x-show="quantity > currentStock" class="p-3 text-xs text-red-800 bg-red-100 rounded-lg dark:text-red-200 dark:bg-red-900/30">
                        Peringatan: Jumlah yang diminta melebihi stok yang tersedia.
                    </div>
                </div>
                 <div class="pt-6 mt-6 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="flex items-center justify-center w-full px-6 py-3 font-semibold text-white transition-all bg-cyan-600 rounded-lg shadow-md hover:bg-cyan-700 focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500" :disabled="quantity > currentStock || quantity < 1">
