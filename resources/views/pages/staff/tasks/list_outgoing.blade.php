...
@section('content')
    ...
    <div class="w-full p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <form action="{{ route('staff.stock.outgoing.list') }}" method="GET">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @forelse($transactions as $transaction)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3"><p class="font-semibold">{{ optional($transaction->product)->name ?? 'N/A' }}</p></td>
                                ...
                                <td class="px-4 py-3">
                                    @if($transaction->status == 'pending')
                                        <a href="{{ route('staff.stock.outgoing.prepare', $transaction) }}" class="px-3 py-1 text-sm font-medium text-white bg-red-600 rounded-md">Siapkan</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center">Data tidak ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
