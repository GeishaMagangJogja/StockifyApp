...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mb-1 text-2xl font-bold text-gray-900 dark:text-white">{{ $title }}</h1>
        <p class="text-gray-600 dark:text-gray-400">Isi form berikut untuk {{ strtolower($title) }} supplier</p>
    </div>
            ...
            <form action="{{ $action }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $supplier->name ?? '') }}" required
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-600
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="3" required
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-600
                                  @error('address') border-red-500 @enderror">{{ old('address', $supplier->address ?? '') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end pt-6 mt-8 space-x-3 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('admin.suppliers.index') }}"
                       class="px-4 py-2 text-gray-700 transition duration-200 border border-gray-300 rounded-lg dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                        <i class="mr-2 fas fa-times"></i> Batal
                    </a>
                    <button type="submit"
                            class="px-4 py-2 text-white transition duration-200 bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
