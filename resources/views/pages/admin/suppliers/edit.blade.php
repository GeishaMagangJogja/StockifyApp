    ...
    <div class="py-8">
        <div>
            <div class="flex items-center space-x-3">
                ...
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Supplier: {{ $supplier->name }}</h1>
            </div>
        </div>
                ...
                <form action="{{ route('admin.suppliers.update', $supplier) }}" method="POST">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $supplier->name) }}" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white" required>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="email" id="email" name="email" value="{{ old('email', $supplier->email) }}" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white" required>
                            </div>
                            <div>
                                ...
                                <input type="tel" id="phone" name="phone" value="{{ old('phone', $supplier->phone) }}" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white" required>
                            </div>
                        </div>

                        <div>
                            <label for="address" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Alamat (Opsional)</label>
                            <textarea id="address" name="address" rows="3" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white">{{ old('address', $supplier->address) }}</textarea>
                        </div>
                    </div>
